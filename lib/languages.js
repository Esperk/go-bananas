...
Languages.init = function(callback) {
	var translations = [__root + 'panel/translations/'],
		self = this;
	...
	async.waterfall([
		...
		function(langId, langPack, callback) {
			...
			async.each(keys, function (key, callback) {
				...
				callback(null, langId, keys);
			});
		},
		function(langId, keys, callback) {
			Translation.find().distinct('object_key', function(err, dbKeys) {
				if (err) {
					callback(err);
				}
				var outdated = [];
				for (var key in dbKeys) {
					if (!~keys.indexOf(dbKeys[key])) {
						outdated.push(dbKeys[key]);
						process.stdout.write(['[', lang.green, ']', '[', dbKeys[key].cyan, ']', 'deleted'.red].join(' ') + '\n');
					}
				}
				callback(null, langId, outdated);
			});
		}, function(langId, keys, callback) {
			Translation.remove({
				_language: langId,
		...
		function(data, callback) {
			if (!data) {
				...
				trans.save(function (err) {
					...
					callback(null, ['[', lang.green, ']', '[', key.cyan, ']', 'added'.green].join(' '));
				});
			} else if (data.object_values !== JSON.stringify(pack)) {
				Translation.update({
						_id: data._id
					}, {
						$set: {
						...
						callback(null, ['[', lang.green, ']', '[', key.cyan, ']', 'updated'.yellow].join(' '));
					});
			} else {
				callback();
			}
